...
describe("GFF tests", () => {
  ...
  describe("Text GFF fetcher", () => {
    ...
    it("should exclude genes", (done) => {
      ...
      df1.fetchTilesDebounced(
        (tiles) => {
          expect(tiles["0.0"][0].fields[8]).to.eql("region");
          expect(tiles["0.0"][0].fields[3]).to.eql("");
          done();
        },
        ["0.0"]
      );
